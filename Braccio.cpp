/*
 ...
 This library was modified by Khulood Alawadi for Robotbae Gizmo to add offset values to calibrate servo motors vShoulder, vElbow and vWrist
 */
...
int _Braccio::ServoMovement(int stepDelay, int vBase, int vShoulder, int vElbow,int vWrist_ver, int vWrist_rot, int vgripper) {
    ...
    // Offset Values
    int vShoulder_offset=7;
    int vElbow_offset=6;
    int vWrist_ver_offset=6;
    
    //Offset Corrections    
    vShoulder=vShoulder+vShoulder_offset;
    vElbow=vElbow+vElbow_offset;
    vWrist_ver=vWrist_ver+vWrist_ver_offset;
	...
	if (vShoulder > 165+vShoulder_offset) vShoulder=165;
	if (vElbow < 0) vElbow=0;
	if (vElbow > 180+vElbow_offset) vElbow=180;
	if (vWrist_ver < 0) vWrist_ver=0;
	if (vWrist_ver > 180+vWrist_ver_offset) vWrist_ver=180;
